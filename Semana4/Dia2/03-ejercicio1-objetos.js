...
/**
 * funcion que retorna la cantidad de peliculas en el idioma ingles
 */
function titulosPorIdioma(idioma) {
  console.log(`Total de peliculas ${peliculas.length}`);
  let contador = 0;
...
console.log(`Cantidad de peliculas en el idioma buscado : ${titulosPorIdioma("en")}`);


/**
 * Funcion que imprime el nombre de la pelicula con mas 
 * generos y la cantidad de generos que tiene 
 * RETO: Mostrar la cantidad de peliculas que tienen la 
 * mayor cantidad de generos en caso que hubiera mas de una
 */
function funcionConMasGeneros() {
  let generos = 0;
  let nombre = "";
  for (const peli of peliculas) {
    if(generos<peli.genre_ids.length){
      generos = peli.genre_ids.length;
      nombre = peli.original_title;
    }
  }
  console.log(`la pelicula con mas generos es: ${nombre} con: ${generos} generos`);
}
funcionConMasGeneros();